...
{
  private:
    // volatile: notify() is called from the BLE task (Core 0), loop() runs on Core 1.
    // Without volatile the compiler may cache these in a register and Core 1 never
    // sees the update. volatile also prevents instruction reordering that could cause
    // loop() to read _eventType before notify() has written it.
    volatile bool _notified = false;
    volatile Nuki::EventType _eventType;
